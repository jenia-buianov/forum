    ...
    foreach ($categories as $category => $item) {
        ...
        <div class="forabg">
            <div class="inner">
                ...
                if (count($item['child']) > 0) {
                    ...
                    <ul class="topiclist forums">
                        ...
                        foreach ($item['child'] as $i => $v) {
                            ?>
                            <li class="row">
                                <dl class="row-item <?php if ($v['unread']) echo 'forum_unread'; elseif ($v['locked']) echo 'forum_read_locked';
                                else  echo 'forum_read'; ?>">
                ...
                -->
                                        <a href="<?= base_url('category/view/' . $v['url']) ?>"
                                           class="forumtitle"><?= $v['title'] ?></a>
                                        <br>
                                        <?= $v['desc'] ?>
                                ...
                                <span><?php
                                    if (!empty($v['last'])) {
                                        ...
                                        <dfn>Last post</dfn>
                                        <a href="<?= base_url('topic/view/' . $v['last']['url']) ?>"
                                           title="<?= $v['last']['title'] ?>"
                                           class="lastsubject"><?= $v['last']['title'] ?></a> <br>
                                        <a href="<?= base_url('users/view/' . $v['last']['userId']) ?>"
                                           style="color: #AA0000;"
                                           class="username-coloured"><?= $v['last']['name'] ?></a>
